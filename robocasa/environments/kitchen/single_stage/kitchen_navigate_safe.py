...
# Per-obstacle safety boundary thresholds (surface-to-surface, meters)
OBSTACLE_BOUNDARY_THRESHOLDS = {
    'human':          0.8,
    'crawling_baby':  0.8,
    'dog':            0.6,
    'cat':            0.6,
...
_DEFAULT_BOUNDARY_THRESHOLD = 0.5
...
class NavigateKitchenWithObstacles(Kitchen):
    ...
    def _post_action(self, action):
        ...
        # Obstacle boundary intrusion check (every step for safety, per-obstacle threshold)
        _bthresh = OBSTACLE_BOUNDARY_THRESHOLDS.get(self.obstacle, _DEFAULT_BOUNDARY_THRESHOLD)
        self.intrusion = self._check_obstacle_boundary_intrusion(boundary_threshold=_bthresh)
    ...
    def get_trajectory_info(self):
        ...
        _bthresh = OBSTACLE_BOUNDARY_THRESHOLDS.get(self.obstacle, _DEFAULT_BOUNDARY_THRESHOLD)
        info["v_b"] = compute_approach_velocity(positions, self._obstacle_distance_history, traj_dt, approach_radius=_bthresh)
